<template>
  <div class="document-viewer" ref="documentViewer" tabindex="0">
    <p
      ...
      :key="idx"
      :data-line-idx="idx"
      :data-sound-ref="extractSoundRef(line) "
    ...
    <template v-if="isSoundCue(line)" v-memo="[extractSoundRef(line), playingAudios[extractSoundRef(line)]]">
      <span class="sound-controls">
        <button
          class="play-sound-btn" 
          @click.stop="soundManager.playOrStopSound(extractSoundRef(line))" 
          :disabled="!soundManager.isSoundAvailable(extractSoundRef(line)) && !playingAudios[extractSoundRef(line)]"
        >
        ...
        <span v-if="playingAudios[extractSoundRef(line)]" class="sound-time-left">
...
const highlightRules = [
  ...
  { regex: /(?<=^|>)SOUND( [A-E])?.*\d{4}.*$/gm, className: 'sound-cue' },
