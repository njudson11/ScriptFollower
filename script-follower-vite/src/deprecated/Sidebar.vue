  ...
  <div class="sidebar">
    ...
    >        
      <span class="sound-controls" v-if="isSoundCue(item.text)">
          <button
            class="play-sound-btn" @click.stop="soundManager.playOrStopSound(item.soundRef)" :disabled="!soundManager.isSoundAvailable(item.soundRef) && !soundManager.isPlaying(item.soundRef)" >
            <span v-if="playingAudios[item.soundRef]">
