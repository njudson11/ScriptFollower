...
- **Sound Management**: Integrates `SoundProcessor` and `WebAudioSoundManager` for handling audio cues. The `WebAudioSoundManager` handles advanced playback options based on properties set on the sound cue line (e.g., stopping all other cues or stopping the previous cue), and provides stereo panning functionality via the Web Audio API.
